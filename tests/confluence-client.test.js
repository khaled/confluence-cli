...
describe('ConfluenceClient', () => {
  ...
  describe('storageToMarkdown', () => {
    ...
    test('should preserve html anchor links in storage content', () => {
      ...
      expect(result).toContain('See [this example change](https://example.com/changes/346578)');
    });
  });
  ...
  describe('htmlToMarkdown', () => {
    ...
    test('should convert html anchors to markdown links', () => {
      const html = '<p>See <a href="https://example.com/changes/347251">this example change</a></p>';
      const result = client.htmlToMarkdown(html);

      expect(result).toContain('See [this example change](https://example.com/changes/347251)');
    });
  });
  ...
  describe('attachments', () => {
    ...
    test('normalizeAttachment should include api context path for relative download links', () => {
      ...
      expect(result.downloadLink).toBe('https://docs.example.com/wiki/download/attachments/123/image.png');
    });
    ...
    test('listAttachments should use response context path when normalizing download links', async () => {
      ...
      expect(response.results[0].downloadLink).toBe(`https://${client.domain}/wiki/download/attachments/123/report.pdf`);
