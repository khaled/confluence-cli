...
class ConfluenceClient {
  ...
  storageToMarkdown(storage, options = {}) {
    ...
    const attachmentHref = (filename) => `${attachmentsDir}/${encodeURIComponent(filename)}`;
    ...
    markdown = markdown.replace(/<ac:image[^>]*>\s*<ri:attachment\s+ri:filename="([^"]+)"[^>]*\s*\/>\s*<\/ac:image>/g, (_, filename) => {
      return `![${filename}](${attachmentHref(filename)})`;
    });
    ...
    markdown = markdown.replace(/<ac:structured-macro ac:name="view-file"[^>]*>[\s\S]*?<ac:parameter ac:name="name">[\s\S]*?<ri:attachment\s+ri:filename="([^"]+)"[^>]*\/>[\s\S]*?<\/ac:parameter>[\s\S]*?<\/ac:structured-macro>/g, (_, filename) => {
      return `\n📎 [${filename}](${attachmentHref(filename)})\n`;
    });
  ...
  htmlToMarkdown(html) {
    ...
    markdown = markdown.replace(/<strong[^>]*>(.*?)<\/strong>/g, (_, text) => `**${text.trim()}**`);
    ...
    // Convert lists BEFORE paragraphs with nesting support.
    // Regex-based list conversion breaks when nested lists are present because
    // it cannot reliably match balanced <ul>/<ol> tags.
    const findTagEnd = (input, tagName, start) => {
      let depth = 0;
      let i = start;
    ...
    const parseListNode = (input, start) => {
      const isOrdered = input.startsWith('<ol>', start);
      const tagName = isOrdered ? 'ol' : 'ul';
      const closeIndex = findTagEnd(input, tagName, start);
      if (closeIndex === -1) {
        return null;
      }
      const content = input.slice(start + tagName.length + 2, closeIndex - (tagName.length + 3));
      const items = [];
      let i = 0;

      while (i < content.length) {
        const liStart = content.indexOf('<li>', i);
        if (liStart === -1) {
          break;
        }
        i = liStart + 4;
        ...
        while (i < content.length && liDepth > 0) {
          if (content.startsWith('<li>', i)) {
            liDepth += 1;
            textBuffer += '<li>';
            i += 4;
            continue;
          ...
          if (liDepth === 1 && (content.startsWith('<ul>', i) || content.startsWith('<ol>', i))) {
            const child = parseListNode(content, i);
            if (!child) {
        ...
        const text = cleanListText(textBuffer);
        items.push({ text, children });
      }

      return {
        node: { type: tagName, items },
        nextIndex: closeIndex
      };
    };

    const renderListNode = (node, depth = 0) => {
      const indent = '  '.repeat(depth);
      const lines = [];
      let counter = 1;

      node.items.forEach((item) => {
        const marker = node.type === 'ol' ? `${counter++}. ` : '- ';
        if (item.text) {
          lines.push(`${indent}${marker}${item.text}`);
        } else if (item.children.length > 0) {
          lines.push(`${indent}${marker.trim()}`);
        }
    ...
    const replaceTopLevelLists = (input) => {
      let result = '';
      let i = 0;
      while (i < input.length) {
        if (input.startsWith('<ul>', i) || input.startsWith('<ol>', i)) {
          const parsed = parseListNode(input, i);
          if (!parsed) {
            result += input[i];
            i += 1;
            continue;
          }
          const rendered = renderListNode(parsed.node);
          result += rendered ? `\n${rendered}\n` : '';
          i = parsed.nextIndex;
          continue;
        }
    ...
    // Trim accidental spaces just inside markdown emphasis delimiters
    // (commonly introduced when stripping style wrappers like <span>).
    markdown = markdown.replace(/\*\*\s+([^\n]*?)\s+\*\*/g, '**$1**');
    markdown = markdown.replace(/(?<!\*)\*\s+([^\n]*?)\s+\*(?!\*)/g, '*$1*');
    markdown = markdown.replace(/__\s+([^\n]*?)\s+__/g, '__$1__');
    markdown = markdown.replace(/(?<!_)_\s+([^\n]*?)\s+_(?!_)/g, '_$1_');
    
    // Clean up extra whitespace for standard Markdown format
    // Remove trailing spaces from each line
    markdown = markdown.replace(/[ \t]+$/gm, '');
    // Remove leading spaces from lines except markdown constructs that rely on indentation.
    markdown = markdown
      .split('\n')
      .map((line) => {
    ...
    // Collapse repeated inline spaces while preserving indentation.
    // This keeps nested list indentation stable for markdown renderers.
    markdown = markdown.replace(/(?<=\S)[ \t]{2,}(?=\S)/g, ' ');
    // Final trim
